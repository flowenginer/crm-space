-- Update function to execute on_reply_action based on the CURRENT STEP's configuration
-- instead of the template's global on_reply_action
...
COMMENT ON FUNCTION public.execute_rescue_on_reply_action() IS 'Executes on_reply_action from the current step when client responds. Falls back to template-level action for backwards compatibility.';