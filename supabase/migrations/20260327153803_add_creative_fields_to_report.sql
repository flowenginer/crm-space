...
-- Add creative/ad fields to search_conversations_report
-- Now exports: creative_name, ad_headline, campaign_name, adset_name, ad_body, source_id
...
RETURNS TABLE(
  id uuid, protocol_number text, status text, lead_status text, close_reason text,
  created_at timestamptz, closed_at timestamptz, first_response_at timestamptz,
  total_active_time_seconds integer, contact_id uuid, contact_full_name text,
  contact_phone text, contact_origin text, contact_lead_status text, contact_lead_score integer,
  channel_id uuid, channel_name text, agent_id uuid, agent_name text,
  department_id uuid, department_name text, referral_source_app text, referral_source_url text,
  creative_name text, ad_headline text, campaign_name text, adset_name text, ad_body text, source_id text,
  tags text, tag_ids text, first_message text,
  sent_messages_count bigint, received_messages_count bigint, internal_notes_text text,
  total_count bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    -- Creative/Ad enriched fields
    COALESCE(
      NULLIF(p.referral_data->>'adName', ''),
      NULLIF(p.referral_data->>'ad_name', ''),
      NULLIF(p.referral_data->>'utm_content', '')
    ) AS creative_name,
    COALESCE(
      NULLIF(p.referral_data->>'headline', '')
    ) AS ad_headline,
    COALESCE(
      NULLIF(p.referral_data->>'campaignName', ''),
      NULLIF(p.referral_data->>'campaign_name', ''),
      NULLIF(p.referral_data->>'utm_campaign', '')
    ) AS campaign_name,
    COALESCE(
      NULLIF(p.referral_data->>'adsetName', ''),
      NULLIF(p.referral_data->>'adset_name', ''),
      NULLIF(p.referral_data->>'utm_medium', '')
    ) AS adset_name,
    COALESCE(
      NULLIF(p.referral_data->>'body', ''),
      NULLIF(p.referral_data->>'greetingMessageBody', '')
    ) AS ad_body,
    COALESCE(
      NULLIF(p.referral_data->>'sourceId', ''),
      NULLIF(p.referral_data->>'source_id', '')
    ) AS source_id,
