-- Corrige o relatório de Atendimentos (search_conversations_report) para filtrar
-- pela data da ÚLTIMA INTERAÇÃO da conversa em vez da data de abertura.
--
-- Problema: quando um lead antigo (conversa nunca fechada) volta a interagir hoje
-- (ex: clicando em um anúncio/campanha), a conversa existente é reaproveitada e seu
-- created_at continua sendo do passado. Como o relatório filtrava por created_at,
-- esse atendimento de hoje não aparecia no relatório do dia.
--
-- Também expõe last_interaction_at no retorno da função para exibição no relatório.
...
BEGIN
  ...
  WITH filtered_conversations AS (
    SELECT
      ...
      c.referral_data,
      c.referral_source,
      ROW_NUMBER() OVER (ORDER BY COALESCE(c.last_client_message_at, c.last_message_at, c.created_at) DESC) AS rn,
      COUNT(*) OVER () AS total_count
    FROM conversations c
    LEFT JOIN contacts co_filter ON co_filter.id = c.contact_id
    WHERE c.tenant_id = v_tenant_id
      -- Filtra pela data da ÚLTIMA INTERAÇÃO (não pela data de abertura da conversa).
      -- Isso evita que um lead antigo, cuja conversa nunca foi fechada, "suma" do
      -- relatório do dia quando ele volta a interagir hoje (ex: via campanha).
      AND (v_start_date IS NULL OR COALESCE(c.last_client_message_at, c.last_message_at, c.created_at) >= v_start_date)
      AND (v_end_date IS NULL OR COALESCE(c.last_client_message_at, c.last_message_at, c.created_at) <= v_end_date)
