-- Atualizar função transfer_conversation para também atualizar o departamento do contato
-- Isso garante que quando uma conversa é transferida, o contato também muda de departamento
-- Tudo em uma única transação (atomicidade garantida)
...
BEGIN
  ...
  -- OTIMIZAÇÃO: Atualizar o departamento do contato na mesma transação
  -- Usa subquery inline para evitar SELECT separado
  IF v_contact_id IS NOT NULL AND v_final_department_id IS NOT NULL THEN
    UPDATE contacts
    SET 
      department_id = v_final_department_id,
      updated_at = NOW()
