...
-- ============================================================
-- CORREÇÃO: Remove ambiguidade de função duplicada
-- search_conversations_report tinha 2 versões simultâneas
-- OID 511461 (antiga: timestamptz + uuid[])
-- OID 511494 (nova: text + text[])
-- ============================================================

-- 1. Remove versão ANTIGA (com timestamptz e uuid[])
...
-- 2. Remove versão NOVA (com text[]) para recriar limpa
DROP FUNCTION IF EXISTS public.search_conversations_report(
  text,
  text,
  text,
...
-- 3. Recria a função única e correta com todos os campos
...
DECLARE
  ...
  v_offset integer;
BEGIN
  -- Resolve tenant_id do usuário autenticado
  ...
  SELECT
    p.id,
    -- Protocol number from conversation_events
    (
    ...
    -- Referral source from referral_data JSON or referral_source column
    COALESCE(
      p.referral_data->>'source',
      p.referral_source
    ) AS referral_source_app,
    COALESCE(
      p.referral_data->>'headline',
      p.referral_data->>'ad_title'
  ...
  WHERE
    (p_name IS NULL OR co.full_name ILIKE '%' || p_name || '%')
...
-- 4. Garante permissão de execução para authenticated
