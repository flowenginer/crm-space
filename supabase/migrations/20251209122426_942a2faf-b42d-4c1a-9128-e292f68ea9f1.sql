-- Corrigir get_all_conversation_counts para tratar channel_id NULL
...
BEGIN
  ...
  -- Channel counts - CORRIGIDO: usar COALESCE para tratar NULL
  SELECT COALESCE(jsonb_object_agg(COALESCE(channel_id_str, 'no_channel'), cnt), '{}'::jsonb) INTO channel_counts
  FROM (
    SELECT COALESCE(c.channel_id::text, 'no_channel') as channel_id_str, COUNT(DISTINCT ct.id) as cnt
    FROM conversations c
    INNER JOIN contacts ct ON ct.id = c.contact_id
    WHERE c.status IN ('open', 'pending')
      AND (p_department_id IS NULL OR c.department_id = p_department_id)
  ...
  -- Department counts - CORRIGIDO: usar COALESCE para tratar NULL
  SELECT COALESCE(jsonb_object_agg(COALESCE(dept_id_str, 'no_department'), cnt), '{}'::jsonb) INTO department_counts
  FROM (
    SELECT COALESCE(c.department_id::text, 'no_department') as dept_id_str, COUNT(DISTINCT ct.id) as cnt
    FROM conversations c
    INNER JOIN contacts ct ON ct.id = c.contact_id
    WHERE c.status IN ('open', 'pending')
      AND (p_agent_id IS NULL OR c.assigned_to = p_agent_id)
  ...
  -- Date counts - CORRIGIDO: contar apenas contatos com conversas ATIVAS
  SELECT jsonb_build_object(
    'today', (
...
-- Corrigir get_date_filter_counts para contar apenas contatos com conversas ATIVAS
...
BEGIN
  ...
  -- Contar CONTATOS únicos com CONVERSAS ATIVAS de HOJE
