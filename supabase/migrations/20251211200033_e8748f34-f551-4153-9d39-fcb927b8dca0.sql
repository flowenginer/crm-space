-- Atualizar função get_all_conversation_counts para usar contacts.first_contact_at
...
BEGIN
  ...
  -- CORRIGIDO: usar ct.first_contact_at ao invés de c.created_at
  ...
  -- Date counts - CORRIGIDO: usar ct.first_contact_at
  SELECT jsonb_build_object(
    'today', (
    ...
    'yesterday', (
      ...
      INNER JOIN conversations c ON c.contact_id = ct.id
      WHERE c.status IN ('open', 'pending')
        AND ct.first_contact_at >= yesterday_start 
        AND ct.first_contact_at < today_start 
        AND (p_department_id IS NULL OR c.department_id = p_department_id) 
        AND (p_agent_id IS NULL OR c.assigned_to = p_agent_id)
    ),
    ...
    'last_week', (
      ...
      INNER JOIN conversations c ON c.contact_id = ct.id
      WHERE c.status IN ('open', 'pending')
        AND ct.first_contact_at >= last_week_start 
        AND ct.first_contact_at < week_start 
        AND (p_department_id IS NULL OR c.department_id = p_department_id) 
        AND (p_agent_id IS NULL OR c.assigned_to = p_agent_id)
    ),
    ...
    'last_month', (
      ...
      INNER JOIN conversations c ON c.contact_id = ct.id
      WHERE c.status IN ('open', 'pending')
        AND ct.first_contact_at >= last_month_start 
        AND ct.first_contact_at < month_start 
        AND (p_department_id IS NULL OR c.department_id = p_department_id) 
        AND (p_agent_id IS NULL OR c.assigned_to = p_agent_id)
    )
...
-- Atualizar função get_date_filter_counts para usar contacts.first_contact_at
CREATE OR REPLACE FUNCTION public.get_date_filter_counts(p_department_id uuid DEFAULT NULL::uuid, p_agent_id uuid DEFAULT NULL::uuid, p_channel_id uuid DEFAULT NULL::uuid, p_origin text DEFAULT NULL::text, p_timezone text DEFAULT 'America/Sao_Paulo'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
