-- Drop the old function with exact signature
...
-- Recreate with correct signature and fixed conversions JOIN
...
BEGIN
  ...
  -- FIXED: Join with lead_statuses to compare name (TEXT) with lead_status (TEXT)
  conversions AS (
    SELECT DISTINCT fc.contact_id
    FROM filtered_conversations fc
    INNER JOIN company_settings cs ON true
    INNER JOIN lead_statuses ls ON ls.id::text = ANY(cs.conversion_status_ids)
