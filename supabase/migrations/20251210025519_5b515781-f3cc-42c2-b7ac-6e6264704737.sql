-- Drop the existing function and recreate with new parameters
...
CREATE OR REPLACE FUNCTION public.search_conversations_report(
  ...
  p_agent_ids text[] DEFAULT NULL::text[],  -- Changed to text[] to allow 'no_agent'
  p_department_ids text[] DEFAULT NULL::text[],  -- Changed to text[] to allow 'no_department'
  p_tag_ids uuid[] DEFAULT NULL::uuid[], 
  p_conversation_status text[] DEFAULT NULL::text[],  -- NEW: filter by conversation status
...
BEGIN
  ...
  -- Check for special 'no_agent' value and extract real agent UUIDs
  ...
  -- Check for special 'no_department' value and extract real department UUIDs
  ...
  WHERE
    ...
    -- Agent filter: handle 'no_agent' + real agents
    AND (
      p_agent_ids IS NULL 
    ...
    -- Department filter: handle 'no_department' + real departments
    AND (
    ...
    -- NEW: Conversation status filter
