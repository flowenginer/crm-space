...
-- Migration definitiva: corrigir referral_source_url para cobrir todos os formatos de dados

-- Drop de TODAS as assinaturas existentes para garantir limpeza completa
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    ...
    FROM pg_proc
    WHERE proname = 'search_conversations_report'
...
-- Recriar função com COALESCE corrigido que cobre ctwa_clid e meta_ads formats
...
BEGIN
  ...
  SELECT
    ...
    -- Fonte do anúncio: campo 'source' ou referral_source da conversa
    COALESCE(
      NULLIF(p.referral_data->>'source', ''),
      NULLIF(p.referral_data->>'sourceApp', ''),
      NULLIF(p.referral_source, '')
    ) AS referral_source_app,
    -- URL do anúncio: cobre ctwa_ad (snake_case), meta_ads (camelCase) e redirect (utm)
    COALESCE(
      NULLIF(p.referral_data->>'video_url', ''),    -- ctwa_ad: URL do vídeo/reel
      NULLIF(p.referral_data->>'source_url', ''),   -- ctwa_ad: URL do post Instagram
      NULLIF(p.referral_data->>'sourceUrl', ''),    -- meta_ads (camelCase): URL do post
      NULLIF(p.referral_data->>'utm_content', ''),  -- redirect: nome do criativo
      NULLIF(p.referral_data->>'utm_medium', '')    -- redirect: nome do conjunto
    ...
    (
      SELECT STRING_AGG(ct.tag_id::text, ',' ORDER BY ct.tag_id::text)
      FROM conversation_tags ct
      WHERE ct.conversation_id = p.id
    ) AS tag_ids,
...
GRANT EXECUTE ON FUNCTION public.search_conversations_report(
