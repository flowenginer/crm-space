...
export function QuoteModal({ open, onOpenChange, quote, conversationId, contactId: initialContactId, channelId, onQuoteCreated }: QuoteModalProps) {
  const { user } = useAuth();
  ...
  // Saved quote state (for WhatsApp sending)
  const [savedQuote, setSavedQuote] = useState<Quote | null>(null);
  const [showSendModal, setShowSendModal] = useState(false);
  const [resolvedChannelId, setResolvedChannelId] = useState<string | null>(channelId || null);
  ...
  // Fetch channel from conversation if not provided
  useEffect(() => {
    if (open && conversationId && !channelId) {
      const fetchChannel = async () => {
        ...
        if (data?.channel_id) {
          setResolvedChannelId(data.channel_id);
        }
      };
      fetchChannel();
    }
  ...
  useEffect(() => {
    if (open && quote) {
      ...
      setIsFreeShipping((quote as any).is_free_shipping || quote.shipping_cost === 0);
  ...
  const total = subtotalAfterDiscount + actualShippingCost;
  ...
  const handleSubmit = async () => {
    ...
    if (isEditMode && quote) {
      ...
      if (result?.id) {
        ...
        // Save the quote for WhatsApp sending - stay open
        setSavedQuote({
          ...result,
          contact: selectedContact ? { id: selectedContact.id, full_name: selectedContact.full_name, phone: selectedContact.phone } : null,
        } as Quote);
      }
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[95vh]">
        <DialogHeader>
          <DialogTitle>
            {savedQuote 
              ? `Orçamento ${savedQuote.quote_number} Criado!` 
          ...
          <div className="py-8 space-y-6">
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <h3 className="text-lg font-semibold">Orçamento criado com sucesso!</h3>
                <p className="text-muted-foreground">
                  {savedQuote.quote_number} • Total: {formatCurrency(total)}
                </p>
                {selectedContact && (
                  <p className="text-sm text-muted-foreground mt-1">
                    Cliente: {selectedContact.full_name}
                  </p>
                )}
              </div>
            </div>
                ...
                <TabsContent value="items" className="space-y-4 px-4">
                  ...
                  <div className="space-y-3 bg-muted/50 p-4 rounded-lg">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className="font-medium text-destructive">-{formatCurrency(totalDiscountValue)}</span>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-3 h-10 px-3 border rounded-md bg-background">
                        ...
                        />
                        <label htmlFor="free-shipping-quote" className="text-sm cursor-pointer select-none">
                          Ativar frete grátis
                        </label>
                      </div>
                    </div>
                ...
                <TabsContent value="payment" className="space-y-4 px-4">
                  ...
                  <div className="p-4 rounded-lg bg-amber-500/10 border border-amber-500/30">
                    <p className="text-sm text-amber-600 dark:text-amber-400">
                      <strong>Nota:</strong> O orçamento não gera lançamento financeiro. 
                      As informações de pagamento são apenas para referência do cliente.
                    </p>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="notes" className="space-y-4 px-4">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      placeholder="Observações que serão exibidas no orçamento para o cliente..."
                      rows={4}
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      placeholder="Notas internas (não serão exibidas para o cliente)..."
                      rows={4}
                    />
                  </div>
                </TabsContent>
      ...
      {savedQuote && (
        <SendDocumentModal
          ...
          documentData={{
            type: 'quote',
            number: savedQuote.quote_number,
            date: format(new Date(), 'dd/MM/yyyy'),
            validUntil: validUntil ? format(new Date(validUntil + 'T12:00:00'), 'dd/MM/yyyy') : undefined,
            contact: {
